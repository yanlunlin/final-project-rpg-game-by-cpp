...
Skill::Skill(string theName, int theDamage, int theMpCost)
    : name(theName), damage(theDamage), mpCost(theMpCost) {}
...
void Skill::setDamage(int theDamage) {
  damage = (theDamage < 0) ? 0 : theDamage;
}
void Skill::setMpCost(int theMpCost) {
  mpCost = (theMpCost < 0) ? 0 : theMpCost;
}

// use the skill
void Skill::use(std::vector<Creature *> team) const {
